...
import '../../provider/hotels_bulk_import_provider.dart';

class HotelsBulkImportScreen extends ConsumerStatefulWidget {
  const HotelsBulkImportScreen({super.key});

  @override
  ConsumerState<HotelsBulkImportScreen> createState() =>
      _HotelsBulkImportScreenState();
}

class _HotelsBulkImportScreenState
    extends ConsumerState<HotelsBulkImportScreen> {
  late final ProviderSubscription<HotelsBulkImportState>
  _pendingImportSubscription;

  @override
  void initState() {
    super.initState();
    _pendingImportSubscription = ref.listenManual(hotelsBulkImportProvider, (
      previous,
      next,
    ) {
  ...
  Future<void> _showConfirmDialog(HotelsPendingImport pending) async {
    final notifier = ref.read(hotelsBulkImportProvider.notifier);
  ...
  Widget build(BuildContext context) {
    final state = ref.watch(hotelsBulkImportProvider);
    final notifier = ref.read(hotelsBulkImportProvider.notifier);

    return ModuleBulkImportLayout(
      title: 'Bulk Import Hotels',
      description:
          'Download the template, fill in your hotels data, and upload to import them all at once.',
