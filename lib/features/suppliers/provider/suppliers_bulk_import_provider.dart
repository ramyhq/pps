...
import 'package:pps/features/suppliers/data/models/supplier_import_row.dart';
import 'package:supabase_flutter/supabase_flutter.dart';
import 'package:universal_html/html.dart' as html;
import 'package:universal_io/io.dart' as io;

class SuppliersBulkImportState {
  static const _unset = Object();

  final List<String> logs;
  final bool isUploading;
  final SuppliersPendingImport? pendingImport;

  const SuppliersBulkImportState({
    this.logs = const [],
    this.isUploading = false,
    this.pendingImport,
  });

  SuppliersBulkImportState copyWith({
    List<String>? logs,
    bool? isUploading,
    Object? pendingImport = _unset,
  }) {
    return SuppliersBulkImportState(
      logs: logs ?? this.logs,
      isUploading: isUploading ?? this.isUploading,
      pendingImport: pendingImport == _unset
          ? this.pendingImport
          : pendingImport as SuppliersPendingImport?,
    );
  }
}

class SuppliersPendingImport {
...
final suppliersBulkImportProvider =
    NotifierProvider.autoDispose<
      SuppliersBulkImportNotifier,
      SuppliersBulkImportState
    >(SuppliersBulkImportNotifier.new);

class SuppliersBulkImportNotifier extends Notifier<SuppliersBulkImportState> {
  @override
  SuppliersBulkImportState build() {
    return const SuppliersBulkImportState(
      logs: ['[System] Ready to import Suppliers...'],
    );
  }
  ...
  Future<void> downloadTemplate() async {
    _addLog('[Process] Generating template for Suppliers...');

    try {
      const csvContent = 'code,name\nS-0001,Global Supplies';
  ...
  Future<void> prepareUpload() async {
    ...
    try {
      ...
      for (final record in records) {
        try {
          final model = SupplierImportRow.fromCsvRecord(record);
      ...
      final dataSource = ref.read(suppliersRemoteDataSourceProvider);
      final codes = <String>{};
      ...
      final existingCodes = await dataSource.findExistingSupplierCodes(codes);
      final pending = SuppliersPendingImport(
        fileName: fileName,
  ...
  Future<void> confirmImport() async {
    ...
    try {
      _addLog('[System] Starting bulk import for Suppliers...');
      _addLog('[Process] Importing records to database...');

      final dataSource = ref.read(suppliersRemoteDataSourceProvider);
      final result = await dataSource.importSuppliers(pending.rows);
      ...
      if (hasIdPkDuplicate) {
        _addLog(
          '[Warning] Database sequence for suppliers.id may be out of sync. In Supabase SQL Editor run: select setval(pg_get_serial_sequence(\'public.suppliers\', \'id\'), (select coalesce(max(id), 0) from public.suppliers) + 1, false);',
        );
      }

      final hasCodeDuplicate = result.sampleErrors.any(
        (e) =>
            e.contains('Key (code)=') ||
            e.contains('suppliers_code_unique') ||
            e.contains('code_unique'),
      );
      if (hasCodeDuplicate) {
        _addLog(
          '[Warning] Some supplier codes already exist in the database. If you expected updates but "Existing codes in DB" shows 0, your current role may not have SELECT access due to RLS policies on public.suppliers.',
        );
      }
