...
import '../../provider/clients_bulk_import_provider.dart';

class ClientsBulkImportScreen extends ConsumerStatefulWidget {
  const ClientsBulkImportScreen({super.key});

  @override
  ConsumerState<ClientsBulkImportScreen> createState() =>
      _ClientsBulkImportScreenState();
}

class _ClientsBulkImportScreenState
    extends ConsumerState<ClientsBulkImportScreen> {
  late final ProviderSubscription<ClientsBulkImportState>
  _pendingImportSubscription;

  @override
  void initState() {
    super.initState();
    _pendingImportSubscription = ref.listenManual(clientsBulkImportProvider, (
      previous,
      next,
    ) {
  ...
  Future<void> _showConfirmDialog(ClientsPendingImport pending) async {
    final notifier = ref.read(clientsBulkImportProvider.notifier);
  ...
  Widget build(BuildContext context) {
    final state = ref.watch(clientsBulkImportProvider);
    final notifier = ref.read(clientsBulkImportProvider.notifier);

    return ModuleBulkImportLayout(
      title: 'Bulk Import Clients',
      description:
          'Download the template, fill in your clients data, and upload to import them all at once.',
