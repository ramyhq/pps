...
import 'package:pps/features/clients/data/models/client_import_row.dart';
import 'package:supabase_flutter/supabase_flutter.dart';
import 'package:universal_html/html.dart' as html;
import 'package:universal_io/io.dart' as io;

class ClientsBulkImportState {
  static const _unset = Object();

  final List<String> logs;
  final bool isUploading;
  final ClientsPendingImport? pendingImport;

  const ClientsBulkImportState({
    this.logs = const [],
    this.isUploading = false,
    this.pendingImport,
  });

  ClientsBulkImportState copyWith({
    List<String>? logs,
    bool? isUploading,
    Object? pendingImport = _unset,
  }) {
    return ClientsBulkImportState(
      logs: logs ?? this.logs,
      isUploading: isUploading ?? this.isUploading,
      pendingImport: pendingImport == _unset
          ? this.pendingImport
          : pendingImport as ClientsPendingImport?,
    );
  }
}

class ClientsPendingImport {
...
final clientsBulkImportProvider =
    NotifierProvider.autoDispose<
      ClientsBulkImportNotifier,
      ClientsBulkImportState
    >(ClientsBulkImportNotifier.new);

class ClientsBulkImportNotifier extends Notifier<ClientsBulkImportState> {
  @override
  ClientsBulkImportState build() {
    return const ClientsBulkImportState(
      logs: ['[System] Ready to import Clients...'],
    );
  }
  ...
  Future<void> downloadTemplate() async {
    _addLog('[Process] Generating template for Clients...');

    try {
      const csvContent = 'code,name\nC-0001,John Doe';
      final bytes = utf8.encode(csvContent);
      const fileName = 'clients_template.csv';
  ...
  Future<void> prepareUpload() async {
    ...
    try {
      ...
      for (final record in records) {
        try {
          final model = ClientImportRow.fromCsvRecord(record);
      ...
      final dataSource = ref.read(clientsRemoteDataSourceProvider);
      final codes = <String>{};
      ...
      final existingCodes = await dataSource.findExistingClientCodes(codes);
      final pending = ClientsPendingImport(
        fileName: fileName,
      ...
      _addLog(
        '[System] Ready to import: ${pending.validRecords} record(s). Existing codes in DB: ${pending.existingCodesInDb}.',
      );
  ...
  Future<void> confirmImport() async {
    ...
    try {
      _addLog('[System] Starting bulk import for Clients...');
      _addLog('[Process] Importing records to database...');

      final dataSource = ref.read(clientsRemoteDataSourceProvider);
      final result = await dataSource.importClients(pending.rows);
