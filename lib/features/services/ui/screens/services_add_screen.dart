...
import 'package:pps/features/services/provider/services_add_provider.dart';

class ServicesAddScreen extends ConsumerStatefulWidget {
  const ServicesAddScreen({super.key});

  @override
  ConsumerState<ServicesAddScreen> createState() => _ServicesAddScreenState();
}

class _ServicesAddScreenState extends ConsumerState<ServicesAddScreen> {
  ...
  Future<void> _submit() async {
    ...
    final notifier = ref.read(servicesAddProvider.notifier);
    ...
    if (error == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Service type saved successfully.')),
      );
      context.go('/services');
      return;
    }
  ...
  Widget build(BuildContext context) {
    final state = ref.watch(servicesAddProvider);
    ...
    return Row(
      children: [
        Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            const Text('Add Service Type', style: AppTextStyles.heading),
  ...
  Widget _buildBottomActions(BuildContext context, {required bool isSaving}) {
    return Row(
      ...
      children: [
        OutlinedButton(
          onPressed: isSaving ? null : () => context.go('/services'),
          style: OutlinedButton.styleFrom(
