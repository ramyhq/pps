...
- `features/<feature>/data`: Models + RemoteDataSources (Supabase/API) مع الحد الأدنى من التعقيد (Repository اختياري فقط للميزات المعقدة).
...
## 6) Data Flow (Target)
UI (screens/widgets) → Provider (Notifier/Controller) → RemoteDataSource → Supabase/API → Provider updates state/logs → UI renders

ملاحظة: Feature `reservations` ما زالت تحتوي مسار أقدم (Repository/DTO) وسيتم تبسيطه لاحقًا عند الحاجة.
